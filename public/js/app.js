///////////////// THIS IS AN OLD CODE : NEEDS TO BE REFACTORED ///////////////////
...
// to be excuted when the equal button is clicked
const url = 'http://localhost:1515';
const header = { 'Content-Type': 'application/json' };

const calculate = async () => {
  // e.preventDefault();
  const data = numInput.value;
  console.log("data is ", data);//1+3
  await fetch("/", {
...
calcButtonsArray.map((clickedBtn) => {
  ...
  clickedBtn.addEventListener("click", (event) => {
    console.log("Hi button clicked", event);
    switch (event.target.classList[1]) { //check the class of the clicked button
      case "number":
        numInput.value += event.target.value; //input the written text of the clicked button
        console.log("I am here numinput value is", numInput.value, "type of numinput", typeof (numInput.value));
        //1+1  string
      ...
      case "equals": //calculate the result --> This will be handled on backend
        console.log(numInput.value);//1+3
        // try {
